    ...
    for i in range(len(layers)):
        ...
        if not args.prune_part:
            for name in subset:
                ...
                elif args.model == "llama2-7b-chat-fp16":
                    W_metric1 = pickle.load(
                        open(
                            f"out/llama2-7b-chat-fp16/unstructured/wandg_weightonly/{metric1}/wanda_score/W_metric_layer_{i}_name_model.layers.{i}.{name}_weight.pkl",
                            "rb",
                        )
                    )
                    W_metric2 = pickle.load(
                        open(
                            f"out/llama2-7b-chat-fp16/unstructured/wandg_weightonly/{metric2}/wanda_score/W_metric_layer_{i}_name_model.layers.{i}.{name}_weight.pkl",
                            "rb",
                        )
                    )
                else:
